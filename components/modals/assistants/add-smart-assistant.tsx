...
const formSchema = z.object({
  ...
  urls: z.array(urlSchema).min(1, { message: 'Список страниц сайта, к данным которых будет обращаться ассистент' }),
});
...
const AddWebPageAssistantItem = () => {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: { name: '', settings: '', comment: '', urls: [''] }
  });

  const { fields, append, remove } = useFieldArray({
    // @ts-ignore
    name: "urls" as any, // Type assertion to any
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-8'>
            <div className='space-y-8 px-6'>
              ...
              {fields.map((field, index) => (
                <div key={field.id} className='flex space-x-2'>
                  <FormItem className='flex-1'>
                    <FormLabel className='uppercase text-xs font-bold text-zinc-500 dark:text-secondary/70'>Ссылка на страницу сайта</FormLabel>
                    <FormControl>
                      <Input
                        disabled={isLoading}
                        className='bg-zinc-300/50 border-0 focus-visible:ring-0 text-black focus-visible:ring-offset-0'
              ...
              <Button type='button' disabled={isLoading} onClick={() => append('')}>Добавить ссылку</Button>
            </div>
